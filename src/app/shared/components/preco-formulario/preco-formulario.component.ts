...
export class PrecoFormularioComponent implements OnInit {
  ...
  tipoChurrascoEnum = TipoChurrasco;
  ...
  submit(): void {
    if (
      ...
      Object.entries(this.formCarnes.value).forEach(([carne, value]) => {
        if (value) {
          this.carnesSelecionadas.push(carne);
        }
      });

      Object.entries(this.formBebidas.value).forEach(([bebida, value]) => {
        if (value) {
